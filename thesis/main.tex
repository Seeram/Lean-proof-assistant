...
\chapter{Teória čiastovného usporiadania}
...
\section{Čiastočné usporiadanie}
\begin{itemize}
...
\begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.30]{cube.png}
...
hotový dôkaz vety:
\begin{lstlisting}
theorem modular_lattice_isomorphism { α: Type u } [ modular_lattice α ] { a b x y : α} :
  (y ≤ b) →
  (x ≥ a) →
  (y ≥ b ⊓ a) →
  (x ≤ b ⊔ a) →
...
u v w x y : α
⊢ x ≤ b → x ≥ a → x ≥ a ⊓ b → x ≤ b ⊔ a → b ⊓ (a ⊔ x) = x ∧ u ⊓ x ⊔ v = x
\end{lstlisting}
